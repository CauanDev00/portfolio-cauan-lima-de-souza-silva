  ...
  const handleDownload = async (extension: 'png' | 'svg' | 'webp') => {
    ...
    try {
      if (settings.mode === 'multi') {
        const node = document.getElementById('qr-plate-capture');
        if (!node) throw new Error("Elemento de captura não encontrado");

        let dataUrl = '';
        if (extension === 'png') dataUrl = await toPng(node, { quality: 1, pixelRatio: 2 });
        else if (extension === 'webp') dataUrl = await toJpeg(node, { quality: 1, pixelRatio: 2 }); // Fallback to Jpeg
        else if (extension === 'svg') dataUrl = await toSvg(node);

        const link = document.createElement('a');
        link.download = `abngrid-plate.${extension}`;
        link.href = dataUrl;
        link.click();
      } else {
      ...
      trackEvent('download_qr', { format: extension, mode: settings.mode });
    } catch (err) {
      console.error("Erro ao baixar:", err);
      alert("Falha ao gerar o download. Tente novamente.");
    } finally {
      setIsDownloading(true); // Wait, should be false
